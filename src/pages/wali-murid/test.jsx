...
const RecapAbsensi = () => {
  ...
  useEffect(() => {
    const fetchDataClassOption = async () => {
      try {
        if (user !== null && user.class !== "") {
          // Set class options and selected class if user.class is not empty
          setClassOptions([{ value: user.class, label: user.class }]);
          ...
          // Set class options
          setClassOptions([
            ...classString.map((className) => ({
              value: className,
              label: className,
            })),
          ]);

          // Set the first available class as selected, or keep it empty
          if (classString.length > 0) {
            setSelectedClass(classString[0]);
          } else {
            setSelectedClass(""); // Reset if no classes available
          }
        } else {
          setClassOptions([
  ...
  }, []); // Add dependencies to re-run when user.class or user.nid changes

  const fetchDataAttendanceRecords = async () => {
    if (selectedDate && selectedClass) {
      try {
        ...
        const attendanceResponse = await axios.get(
          `${
            import.meta.env.VITE_BASE_URL_BACKEND
          }/attendance?class=${encodeURIComponent(selectedClass)}`
        );
        const attendanceRecords = attendanceResponse.data;
        ...
        const permitResponse = await axios.get(
          `${
            import.meta.env.VITE_BASE_URL_BACKEND
          }/permits?class=${encodeURIComponent(selectedClass)}`
        );
        ...
        const processedPermits = permits
          .filter((permit) => permit.status === "Accepted") // Only include accepted permits
          .map((permit) => ({
            ...permit,
        ...
        const formattedData = students.map((student) => {
          ...
          const studentPermits = processedPermits.filter(
            (permit) => permit.name === student.name
          );
          ...
          studentPermits.forEach((permit) => {
            ...
            let method = methodMap[permit.reason.toLowerCase()] || "-";
            attendance[dayIndex] = method;
          });
  ...
  const fetchDataSubjectAttendanceRecords = async () => {
    if (selectedDate && selectedClass) {
      try {
        const classResponse = await axios.get(
          `${
            import.meta.env.VITE_BASE_URL_BACKEND
          }/classes?name=${encodeURIComponent(selectedClass)}`
        );
        ...
        const subjectattendanceResponse = await axios.get(
          `${
            import.meta.env.VITE_BASE_URL_BACKEND
          }/subjectattendance?class=${encodeURIComponent(selectedClass)}`
        );
        const subjectattendanceRecords = subjectattendanceResponse.data;
        ...
        const classSchedule = await axios.get(
          `${import.meta.env.VITE_BASE_URL_BACKEND}/classschedule?teacherid=${
            user.nid
          }`
        );
        const schedule = classSchedule.data;
        ...
        const processedRecords = subjectattendanceRecords.map((record) => ({
          ...record,
          date: moment.unix(record.date).format("YYYY-MM-DD"),
        }));
        ...
        const scheduleClass = schedule.filter(
          (s) => s.class_id === classData[0].id
        );
        ...
        const hasScheduleForDate = (date) => {
          const dayName = new Date(date)
            ...
            .toLowerCase();
          return scheduleClass.some((s) => s.day === dayName);
        };
        ...
        formattedData.forEach((student) => {
          student.attendance = student.attendance.map((status, index) => {
            ...
            const hasSchedule = hasScheduleForDate(currentDate);
          ...
          const effectiveDays = Array.from(
            ...
            (_, i) => {
              const currentDate = moment(selectedDate)
                .date(i + 1)
                .startOf("day");
              return hasScheduleForDate(currentDate) ? 1 : 0; // Jika ada jadwal, hitung sebagai 1
            }
          ).reduce((sum, day) => sum + day, 0); // Jumlahkan hari-hari efektif
